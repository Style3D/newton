...
"""Simulate a plastic bag and drawstring with SolverStyle3D.

Command:
    python -m newton.examples cloth_style3d_plastic_bag_drawstring --device cuda:0
"""
...
def load_obj_mesh(path: Path, *, load_uvs: bool = True) -> ObjMesh:
    vertices: list[list[float]] = []
    texcoords: list[list[float]] = []
    triangles: list[list[int]] = []
    triangle_texcoords: list[list[int]] = []
    ...
    with path.open("r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            elif load_uvs and line.startswith("vt "):
                parts = line.split()
                if len(parts) >= 3:
                    texcoords.append([float(parts[1]), float(parts[2])])
            elif line.startswith("f "):
                face: list[int] = []
                face_texcoords: list[int | None] = []
                for token in line.split()[1:]:
                    token_parts = token.split("/")
                    if not token_parts[0]:
                        continue
                    face.append(_parse_obj_index(token_parts[0], len(vertices)))
                    if load_uvs and len(token_parts) >= 2 and token_parts[1]:
                        face_texcoords.append(_parse_obj_index(token_parts[1], len(texcoords)))
                    else:
                        face_texcoords.append(None)

                for i in range(1, len(face) - 1):
                    triangles.append([face[0], face[i], face[i + 1]])
                    triangle_texcoords.append([face_texcoords[0], face_texcoords[i], face_texcoords[i + 1]])
    ...
    has_complete_uvs = (
        load_uvs
        and len(texcoords) > 0
        and len(triangle_texcoords) == len(triangles)
        and all(all(index is not None for index in tri) for tri in triangle_texcoords)
    )
    if has_complete_uvs:
        panel_vertices = np.asarray(texcoords, dtype=np.float64)
        panel_indices = np.asarray(triangle_texcoords, dtype=np.int32).reshape(-1, 3)
...
def infer_panel_scale(meshes: list[ObjMesh]) -> float:
    ratios: list[float] = []
    for mesh in meshes:
        if mesh.panel_vertices is None:
            continue
        spatial_extent = float(np.max(np.ptp(mesh.vertices, axis=0)))
        panel_extent = float(np.max(np.ptp(mesh.panel_vertices, axis=0)))
        if spatial_extent > 0.0 and panel_extent > 0.0:
            ratios.append(spatial_extent / panel_extent)
    return float(np.median(ratios)) if ratios else 1.0


def cylindrical_panel_vertices(mesh: ObjMesh) -> np.ndarray:
    center = np.mean(mesh.vertices[:, :2], axis=0)
    xy = mesh.vertices[:, :2] - center[None, :]
    radius = float(np.mean(np.linalg.norm(xy, axis=1)))
    radius = max(radius, 1.0e-6)
    theta = np.unwrap(np.arctan2(xy[:, 1], xy[:, 0]))
    return np.stack([theta * radius, mesh.vertices[:, 2]], axis=1)


def pca_panel_vertices(mesh: ObjMesh) -> np.ndarray:
    centered = mesh.vertices - np.mean(mesh.vertices, axis=0)
    _, _, vh = np.linalg.svd(centered, full_matrices=False)
    return centered @ vh[:2].T


def ensure_panel_space(mesh: ObjMesh, *, panel_scale: float, fallback: str) -> ObjMesh:
    if mesh.panel_vertices is not None and mesh.panel_indices is not None:
        return ObjMesh(
            vertices=mesh.vertices,
            indices=mesh.indices,
            panel_vertices=mesh.panel_vertices * panel_scale,
            panel_indices=mesh.panel_indices,
        )

    if fallback == "cylindrical-z":
        panel_vertices = cylindrical_panel_vertices(mesh)
    elif fallback == "pca":
        panel_vertices = pca_panel_vertices(mesh)
    elif fallback == "xy":
        panel_vertices = mesh.vertices[:, [0, 1]]
    elif fallback == "xz":
        panel_vertices = mesh.vertices[:, [0, 2]]
    elif fallback == "yz":
        panel_vertices = mesh.vertices[:, [1, 2]]
    else:
        raise ValueError(f"Unsupported panel fallback: {fallback}")

    return ObjMesh(
        vertices=mesh.vertices,
        indices=mesh.indices,
        panel_vertices=panel_vertices,
        panel_indices=mesh.indices,
    )


def fix_panel_winding(mesh: ObjMesh, *, enabled: bool) -> tuple[ObjMesh, int, int]:
    if not enabled or mesh.panel_vertices is None or mesh.panel_indices is None:
        return mesh, 0, 0

    tri = mesh.panel_vertices[mesh.panel_indices]
    qp = tri[:, 1] - tri[:, 0]
    rp = tri[:, 2] - tri[:, 0]
    area2 = qp[:, 0] * rp[:, 1] - qp[:, 1] * rp[:, 0]
    flipped = area2 < -1.0e-14
    zero = np.abs(area2) <= 1.0e-14
    if not np.any(flipped):
        return mesh, 0, int(np.count_nonzero(zero))

    indices = mesh.indices.copy()
    panel_indices = mesh.panel_indices.copy()
    indices[flipped] = indices[flipped][:, [0, 2, 1]]
    panel_indices[flipped] = panel_indices[flipped][:, [0, 2, 1]]

    tri_repaired = mesh.panel_vertices[panel_indices]
    qp_repaired = tri_repaired[:, 1] - tri_repaired[:, 0]
    rp_repaired = tri_repaired[:, 2] - tri_repaired[:, 0]
    area2_repaired = qp_repaired[:, 0] * rp_repaired[:, 1] - qp_repaired[:, 1] * rp_repaired[:, 0]
    zero_repaired = int(np.count_nonzero(np.abs(area2_repaired) <= 1.0e-14))
    return (
        ObjMesh(
            vertices=mesh.vertices,
            indices=indices,
            panel_vertices=mesh.panel_vertices,
            panel_indices=panel_indices,
        ),
        int(np.count_nonzero(flipped)),
        zero_repaired,
    )


def clean_mesh_faces(
    mesh: ObjMesh,
    *,
    remove_duplicate_faces: bool,
    min_world_area: float,
    min_panel_area: float,
) -> tuple[ObjMesh, int, int, int]:
    keep = np.ones(len(mesh.indices), dtype=bool)
    duplicate_count = 0

    if remove_duplicate_faces:
        seen_faces: set[tuple[int, int, int]] = set()
        for index, face in enumerate(mesh.indices):
    ...
    world_t = mesh.vertices[mesh.indices]
    world_area = 0.5 * np.linalg.norm(
        np.cross(world_t[:, 1] - world_t[:, 0], world_t[:, 2] - world_t[:, 0]),
        axis=1,
    )
    tiny_world = world_area <= min_world_area
    keep &= ~tiny_world

    tiny_panel = np.zeros(len(mesh.indices), dtype=bool)
    if mesh.panel_vertices is not None and mesh.panel_indices is not None:
        panel_t = mesh.panel_vertices[mesh.panel_indices]
        qp = panel_t[:, 1] - panel_t[:, 0]
        rp = panel_t[:, 2] - panel_t[:, 0]
        panel_area = 0.5 * np.abs(qp[:, 0] * rp[:, 1] - qp[:, 1] * rp[:, 0])
        tiny_panel = panel_area <= min_panel_area
        keep &= ~tiny_panel

    if not np.any(keep):
        raise ValueError("Style3D mesh cleanup removed all triangles.")

    panel_indices = mesh.panel_indices[keep] if mesh.panel_indices is not None else None
    return (
        ObjMesh(
            vertices=mesh.vertices,
        ...
        duplicate_count,
        int(np.count_nonzero(tiny_world)),
        int(np.count_nonzero(tiny_panel)),
    )
...
def weld_vertices(mesh: ObjMesh, max_distance: float) -> tuple[ObjMesh, int]:
    ...
    panel_indices = mesh.panel_indices[valid] if mesh.panel_indices is not None else None
    return (
        ObjMesh(
            vertices=np.asarray(new_vertices, dtype=np.float64),
            indices=new_indices[valid],
            panel_vertices=mesh.panel_vertices,
            panel_indices=panel_indices,
        ),
        weld_count,
...
def add_style3d_cloth_obj(
    ...
    *,
    ...
    scale: float,
    density: float,
    tri_aniso_ke: tuple[float, float, float],
    tri_ka: float,
    tri_kd: float,
    edge_aniso_ke: tuple[float, float, float],
    edge_kd: float,
    ...
    style3d.add_cloth_mesh(
        ...
        panel_verts=None if mesh.panel_vertices is None else mesh.panel_vertices.tolist(),
        panel_indices=None if mesh.panel_indices is None else mesh.panel_indices.reshape(-1).tolist(),
        density=density,
        scale=scale,
        tri_aniso_ke=wp.vec3(*tri_aniso_ke),
        tri_ka=tri_ka,
        tri_kd=tri_kd,
        edge_aniso_ke=wp.vec3(*edge_aniso_ke),
        edge_kd=edge_kd,
...
class Example:
    def __init__(self, viewer, args):
        ...
        dustbin_mesh = None if bool(args.no_dustbin) else load_obj_mesh(dustbin_path, load_uvs=False)
        ...
        panel_scale = (
            float(args.style3d_panel_scale)
            if args.style3d_panel_scale is not None
            else infer_panel_scale([bag_mesh, drawstring_mesh])
        )
        ...
        bag_mesh = ensure_panel_space(bag_mesh, panel_scale=panel_scale, fallback=args.bag_panel_fallback)
        drawstring_mesh = ensure_panel_space(
            drawstring_mesh, panel_scale=panel_scale, fallback=args.drawstring_panel_fallback
        )
        bag_mesh, bag_panel_flipped, bag_panel_zero = fix_panel_winding(
            bag_mesh, enabled=not bool(args.no_style3d_fix_panel_winding)
        )
        drawstring_mesh, drawstring_panel_flipped, drawstring_panel_zero = fix_panel_winding(
            drawstring_mesh, enabled=not bool(args.no_style3d_fix_panel_winding)
        )
        bag_mesh, bag_duplicate_faces, bag_tiny_world_faces, bag_tiny_panel_faces = clean_mesh_faces(
            bag_mesh,
            remove_duplicate_faces=bool(args.style3d_clean_duplicate_faces),
            min_world_area=float(args.style3d_min_world_area),
            min_panel_area=float(args.style3d_min_panel_area),
        )
        (
            drawstring_mesh,
            drawstring_duplicate_faces,
            drawstring_tiny_world_faces,
            drawstring_tiny_panel_faces,
        ) = clean_mesh_faces(
            drawstring_mesh,
            remove_duplicate_faces=bool(args.style3d_clean_duplicate_faces),
            min_world_area=float(args.style3d_min_world_area),
        ...
        self.bag_slice, self.bag_tri_slice = add_style3d_cloth_obj(
            builder,
            bag_mesh,
            pos=pos,
            scale=scale,
            density=float(args.bag_density),
            tri_aniso_ke=tuple(map(float, args.bag_tri_aniso_ke)),
            tri_ka=float(args.bag_tri_ka),
            tri_kd=float(args.bag_tri_kd),
            edge_aniso_ke=tuple(map(float, args.bag_edge_aniso_ke)),
            ...
            validate_mesh=bool(args.validate_mesh),
            label="plastic_bag",
        )
        ...
        self.drawstring_slice, self.drawstring_tri_slice = add_style3d_cloth_obj(
            builder,
            drawstring_mesh,
            pos=pos,
            scale=scale,
            density=float(args.drawstring_density),
            tri_aniso_ke=tuple(map(float, args.drawstring_tri_aniso_ke)),
            tri_ka=float(args.drawstring_tri_ka),
            tri_kd=float(args.drawstring_tri_kd),
            edge_aniso_ke=tuple(map(float, args.drawstring_edge_aniso_ke)),
        ...
        self.model.soft_contact_radius = float(args.soft_contact_radius)
        ...
        if self.solver.collision is not None:
            ...
            if bool(args.no_style3d_self_collision):
                self.solver.collision.stiff_vf = 0.0
                self.solver.collision.stiff_ee = 0.0
                self.solver.collision.stiff_ef = 0.0
            else:
                self.solver.collision.stiff_vf = float(args.style3d_collision_stiff_vf)
                self.solver.collision.stiff_ee = float(args.style3d_collision_stiff_ee)
                self.solver.collision.stiff_ef = float(args.style3d_collision_stiff_ef)
        ...
        print(
            ...
            f"scale={scale:g}, panel_scale={panel_scale:g}, "
            ...
            f"bag_panel_flipped={bag_panel_flipped}, drawstring_panel_flipped={drawstring_panel_flipped}, "
            f"bag_cleaned_faces={bag_duplicate_faces + bag_tiny_world_faces + bag_tiny_panel_faces}, "
            f"drawstring_cleaned_faces={drawstring_duplicate_faces + drawstring_tiny_world_faces + drawstring_tiny_panel_faces}",
            flush=True,
        )
        print(
            "[PlasticBagStyle3D] "
            ...
            f"bag_zero_panel_tris={bag_panel_zero}, drawstring_zero_panel_tris={drawstring_panel_zero}, "
            f"bag_duplicate_faces={bag_duplicate_faces}, bag_tiny_world_faces={bag_tiny_world_faces}, "
            f"bag_tiny_panel_faces={bag_tiny_panel_faces}, "
            f"drawstring_duplicate_faces={drawstring_duplicate_faces}, "
            f"drawstring_tiny_world_faces={drawstring_tiny_world_faces}, "
            f"drawstring_tiny_panel_faces={drawstring_tiny_panel_faces}",
            flush=True,
        )
        if bool(args.pull_drawstring) and bool(args.no_cuda_graph) is False:
            print(
                "[PlasticBagStyle3D] CUDA graph disabled because drawstring pull targets are updated each step.",
                flush=True,
            )
        self.capture()
    ...
    def capture(self):
        ...
        if self.sim_substeps % 2:
            print("[PlasticBagStyle3D] CUDA graph disabled because --view-substeps is odd.", flush=True)
            return
        try:
            with wp.ScopedCapture() as capture:
                self.simulate()
            self.graph = capture.graph
            print(f"[PlasticBagStyle3D] CUDA graph capture enabled for {self.sim_substeps} substeps.", flush=True)
        except Exception as exc:
            print(
                f"[PlasticBagStyle3D] CUDA graph capture failed; falling back to Python stepping: {exc!r}", flush=True
            )

    def simulate(self):
        collide_interval = max(1, int(self.args.collide_every_substeps))
        for substep in range(self.sim_substeps):
            time = self.sim_time + substep * self.sim_dt
            self._initialize_pull(time)
            self._drive_pull_particles(self.state_0, time)
            if substep % collide_interval == 0:
                self.collision_pipeline.collide(
                    self.state_0,
                    self.contacts,
                    soft_contact_margin=float(self.args.soft_contact_margin),
                )
    ...
    def render(self):
        ...
        self.viewer.log_mesh(
            "/plastic_bag_style3d/drawstring",
            self.state_0.particle_q,
        ...
        self.viewer.log_points(
            "/plastic_bag_style3d/colored_particles",
            self.state_0.particle_q,
    ...
    def test_final(self):
        ...
        qd = self.state_0.particle_qd.numpy()
        if len(q) == 0:
            raise ValueError("Plastic bag Style3D example has no particles.")
        if not np.isfinite(q).all() or not np.isfinite(qd).all():
            raise ValueError("Plastic bag Style3D particle state is not finite.")
        ...
        if max_abs_position > 10.0 or max_speed > 200.0:
            raise ValueError(
                f"Plastic bag Style3D simulation appears unstable: "
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scale applied to OBJ vt panel coordinates",
    )
    parser.add_argument(
        "--bag-panel-fallback",
        choices=["cylindrical-z", "pca", "xy", "xz", "yz"],
        default="cylindrical-z",
        help="Panel-space generation if the bag OBJ has no usable UVs",
    )
    parser.add_argument(
        "--drawstring-panel-fallback",
        choices=["cylindrical-z", "pca", "xy", "xz", "yz"],
        default="pca",
        help="Panel-space generation if the drawstring OBJ has no usable UVs",
    )
    ...
    parser.add_argument("--collide-every-substeps", type=int, default=1)
    parser.add_argument("--solver-iterations", type=int, default=6)
    parser.add_argument("--linear-iterations", type=int, default=6)
    ...
    parser.add_argument(
        "--style3d-clean-duplicate-faces",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Remove duplicate sorted triangle faces before building Style3D cloth",
    )
    parser.add_argument(
        "--style3d-min-world-area",
        type=float,
        default=0.0,
        help="Drop triangles with 3D area at or below this threshold [m^2]",
    )
    parser.add_argument(
        "--style3d-min-panel-area",
        type=float,
        default=1.0e-14,
        help="Drop triangles with panel-space area at or below this threshold",
    )
    parser.add_argument("--no-cuda-graph", action="store_true")
    parser.add_argument("--log-interval", type=int, default=30)
    parser.add_argument("--validate-mesh", action="store_true")
    ...
    parser.add_argument("--bag-particle-radius", type=float, default=0.003)
    parser.add_argument("--bag-tri-aniso-ke", type=parse_vec3, default=(500.0, 500.0, 100.0))
    parser.add_argument("--bag-tri-ka", type=float, default=50.0)
    parser.add_argument("--bag-tri-kd", type=float, default=1.0e-4)
    parser.add_argument("--bag-edge-aniso-ke", type=parse_vec3, default=(2.0e-5, 2.0e-5, 2.0e-5))
    ...
    parser.add_argument("--drawstring-particle-radius", type=float, default=0.003)
    parser.add_argument("--drawstring-tri-aniso-ke", type=parse_vec3, default=(2.0e3, 2.0e3, 5.0e2))
    parser.add_argument("--drawstring-tri-ka", type=float, default=100.0)
    parser.add_argument("--drawstring-tri-kd", type=float, default=1.0e-4)
    parser.add_argument("--drawstring-edge-aniso-ke", type=parse_vec3, default=(2.0e-4, 2.0e-4, 2.0e-4))
    ...
    parser.add_argument(
        "--pull-duration", type=float, default=2.0, help="Duration of the drawstring pull trajectory [s]"
    )
    parser.add_argument("--pull-height", type=float, default=0.45, help="Final upward drawstring pull distance [m]")
    parser.add_argument(
        "--pull-outward-distance", type=float, default=0.25, help="Final outward drawstring pull distance [m]"
    )
    parser.add_argument(
    ...
    parser.add_argument("--soft-contact-radius", type=float, default=0.005)
    parser.add_argument("--soft-contact-margin", type=float, default=0.012)
    parser.add_argument("--soft-contact-ke", type=float, default=1.0e4)
    parser.add_argument("--soft-contact-kd", type=float, default=1.0e-1)
    parser.add_argument("--soft-contact-mu", type=float, default=0.8)
    parser.add_argument("--dustbin-contact-ke", type=float, default=1.0e5)
    parser.add_argument("--dustbin-contact-kd", type=float, default=1.0e1)
    parser.add_argument("--dustbin-contact-mu", type=float, default=0.7)
    parser.add_argument("--style3d-collision-radius", type=float, default=0.003)
    parser.add_argument("--style3d-collision-stiff-vf", type=float, default=0.02)
    parser.add_argument("--style3d-collision-stiff-ee", type=float, default=0.01)
    parser.add_argument("--style3d-collision-stiff-ef", type=float, default=0.04)
