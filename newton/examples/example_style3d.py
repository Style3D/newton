...
from newton.utils.renderer import PolyscopeRenderer
    ...
    def advance_frame(self):
        with wp.ScopedTimer("step", print=False, dict=self.profiler):
            if self.use_cuda_graph:
                ...
                self.integrate_frame_substeps()
            self.renderer.sim_time += self.frame_dt
    ...
    def update(self):
        if self.renderer.drag_info_chg:
            self.solver.update_drag_info(
                self.renderer.drag_index, self.renderer.drag_position, self.renderer.drag_bary_coord
            )
