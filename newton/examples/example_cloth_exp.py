...
class Example:
    def __init__(self, stage_path="example_cloth_self_contact.usd", num_frames=600):
        ...
        #usd_stage = Usd.Stage.Open(newton.examples.get_asset("Untitled.usda"))
        #usd_geom = UsdGeom.Mesh(usd_stage.GetPrimAtPath("/Root/Untitled/Root_Garment"))

        mesh = newton.utils.load_mesh(newton.examples.get_asset("Plane5K.obj"))
        mesh_points = mesh[0]
        mesh_indices = mesh[1]
        ...
        builder.add_cloth_mesh(
            ...
            rot=wp.quat_from_axis_angle(wp.vec3(1.0, 0.0, 0.0), wp.half_pi * 0.5),
            scale=1.0,
            vertices=vertices,
        ...
        # fix 51 101
        flags = self.model.particle_flags.numpy()
        flags[51] = wp.uint32(int(flags[51]) & ~int(PARTICLE_FLAG_ACTIVE))
        flags[101] = wp.uint32(int(flags[101]) & ~int(PARTICLE_FLAG_ACTIVE))
        ...
        self.renderer = None
        if stage_path:
            self.renderer = newton.utils.SimRendererOpenGL(path=stage_path, model=self.model, scaling=1)
