...
def configure_piper_position_targets(builder: newton.ModelBuilder, joint_q_start: int, joint_q_count: int):
    target_ke = [80.0, 80.0, 80.0, 40.0, 10.0, 10.0, 400.0, 400.0]
    target_kd = [5.0, 5.0, 5.0, 5.0, 1.5, 1.5, 5.0, 5.0]
    for i in range(min(joint_q_count, len(target_ke))):
        q_idx = joint_q_start + i
        builder.joint_target_ke[q_idx] = target_ke[i]
        builder.joint_target_kd[q_idx] = target_kd[i]
        builder.joint_target_mode[q_idx] = int(newton.JointTargetMode.POSITION)
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.tennis_radius = float(np.linalg.norm(tennis_vertices_np - tennis_local_center_np, axis=1).max())
        ...
        self.sim_rigid_body_shape_indices = set()
        self.dynamic_sim_rigid_body_shapes = []

        ik_builder = newton.ModelBuilder(up_axis=newton.Axis.Y)
        (
            self.ik_piper_xform,
            self.ik_piper_body_start,
            self.ik_piper_body_count,
            self.ik_piper_joint_q_start,
            self.ik_piper_joint_q_count,
            self.ik_piper_joint_qd_start,
            self.ik_piper_joint_qd_count,
        ...
        piper_builder = newton.ModelBuilder(up_axis=newton.Axis.Y)
        newton.solvers.SolverMuJoCo.register_custom_attributes(piper_builder)
        (
            self.piper_xform,
            self.piper_body_start,
            self.piper_body_count,
            self.piper_joint_q_start,
            self.piper_joint_q_count,
            self.piper_joint_qd_start,
            self.piper_joint_qd_count,
        ) = add_piper(piper_builder, piper_mjcf_path)
        configure_piper_position_targets(piper_builder, self.piper_joint_q_start, self.piper_joint_q_count)

        self.piper_tennis_body = piper_builder.add_body(
            xform=tennis_body_xform,
            mass=0.1,
            label=tennis_name,
            is_kinematic=True,
        )
        piper_builder.add_shape_mesh(
            label=tennis_name,
            body=self.piper_tennis_body,
            xform=tennis_shape_xform,
            mesh=Mesh(tennis_mesh.vertices, tennis_mesh.indices),
        )
        self.piper_model = piper_builder.finalize()
        self.piper_model.set_gravity((0.0, 0.0, 0.0))
        self.piper_state_0 = self.piper_model.state()
        self.piper_state_1 = self.piper_model.state()
        self.piper_contacts = self.piper_model.contacts()
        self.piper_control = self.piper_model.control()
        wp.copy(
            self.piper_control.joint_target_q[: self.piper_joint_q_count],
            self.piper_model.joint_q[: self.piper_joint_q_count],
        )
        newton.eval_fk(
            self.piper_model,
            self.piper_model.joint_q,
            self.piper_model.joint_qd,
            self.piper_state_0,
        )
        self.piper_state_1.joint_q.assign(self.piper_state_0.joint_q)
        self.piper_state_1.joint_qd.assign(self.piper_state_0.joint_qd)
        self.piper_state_1.body_q.assign(self.piper_state_0.body_q)
        self.piper_state_1.body_qd.assign(self.piper_state_0.body_qd)
        self.piper_solver = newton.solvers.SolverMuJoCo(
            self.piper_model,
        ...
        self.piper_gripper_body_indices_backend = tuple(
            i for i, label in enumerate(self.piper_model.body_label) if label.endswith(PIPER_GRIPPER_BODY_SUFFIXES)
        )
        self.piper_finger_q_indices = tuple(
            int(self.piper_model.joint_q_start.numpy()[i])
        ...
        self.piper_tennis_joint_q_start = int(self.piper_model.joint_q_start.numpy()[self.piper_tennis_joint])
        self.piper_tennis_joint_qd_start = int(self.piper_model.joint_qd_start.numpy()[self.piper_tennis_joint])
        self.ik_joint_q = self.ik_model.joint_q.reshape((1, self.ik_model.joint_coord_count))
        piper_body_q_np = self.piper_state_0.body_q.numpy()
        ik_body_q_np = self.ik_state.body_q.numpy()
        ee_xform = wp.transform(*ik_body_q_np[self.ik_piper_ee_body])
        initial_gripper_center = self._piper_gripper_center_np(piper_body_q_np)
        ...
        self.piper_tennis_shape_indices = set(self.piper_model.body_shapes[self.piper_tennis_body])
        self.piper_gripper_shape_indices = set()
        for body_idx, label in enumerate(self.piper_model.body_label):
            if label.endswith(PIPER_GRIPPER_BODY_SUFFIXES):
                self.piper_gripper_shape_indices.update(self.piper_model.body_shapes[body_idx])
        ...
        self.ik_iters = 18
        self.piper_shape_scale_np = self.piper_model.shape_scale.numpy()
        self.piper_shape_transform_np = self.piper_model.shape_transform.numpy()
        ...
        self.backend_gripper_has_tennis_contact = False
        self.sync_backend_tennis_body_xform(tennis_body_xform)
        ...
        for i in self.sim_rigid_body_indices:
            ...
            for shape_idx in shape_indices:
                if isinstance(self.model.shape_source[shape_idx], newton.Mesh):
                    ...
                    self.sim_rigid_body_shape_indices.add(shape_idx)
                    if i == tennis_body:
        ...
        piper_body_q_np = self.piper_state_0.body_q.numpy()
        piper_shape_flags_np = self.piper_model.shape_flags.numpy()
        for body_idx in self.piper_gripper_body_indices_backend:
            for shape_idx in self.piper_model.body_shapes[body_idx]:
                if piper_shape_flags_np[shape_idx] & collide_shapes_flag == 0:
                    continue
                if not isinstance(self.piper_model.shape_source[shape_idx], newton.Mesh):
                    continue

                shape_label = self.piper_model.shape_label[shape_idx]
                shape_tris_np = np.asarray(
                    self.piper_model.shape_source[shape_idx].indices,
                    dtype=np.int32,
                ).reshape(-1, 3)
                if GRIPPER_AS_RIGIDBODY:
                    ...
                    static_mesh = sim.Mesh(
                        self.piper_model.shape_source[shape_idx].indices.flatten(),
    ...
    def _shape_world_vertices_np(self, shape_idx: int, body_xform_np) -> np.ndarray:
        return self._transform_points_np(self._shape_body_vertices_np(shape_idx), body_xform_np)
    ...
    def _piper_shape_body_vertices_np(self, shape_idx: int) -> np.ndarray:
        ...
        vertices = np.asarray(self.piper_model.shape_source[shape_idx].vertices, dtype=np.float32).reshape(-1, 3)
    ...
    def _piper_gripper_center_np(self, piper_body_q_np: np.ndarray) -> np.ndarray:
        shape_scale_np = self.piper_model.shape_scale.numpy()
        shape_transform_np = self.piper_model.shape_transform.numpy()
        shape_flags_np = self.piper_model.shape_flags.numpy()
        collide_shapes_flag = int(newton.ShapeFlags.COLLIDE_SHAPES)
        ...
        for body_idx in self.piper_gripper_body_indices_backend:
            ...
            for shape_idx in self.piper_model.body_shapes[body_idx]:
                ...
                vertices = np.asarray(
                    self.piper_model.shape_source[shape_idx].vertices,
                    dtype=np.float32,
                ).reshape(-1, 3)
    ...
    def _has_backend_gripper_tennis_contact(self):
        count = int(self.piper_contacts.rigid_contact_count.numpy()[0])
        if count <= 0:
            return False

        shape0 = self.piper_contacts.rigid_contact_shape0.numpy()[:count]
        shape1 = self.piper_contacts.rigid_contact_shape1.numpy()[:count]
        for s0, s1 in zip(shape0, shape1, strict=False):
            pair = {int(s0), int(s1)}
            if pair & self.piper_tennis_shape_indices and pair & self.piper_gripper_shape_indices:
                return True
    ...
    def _write_backend_tennis_state(self, state, xform_np: np.ndarray):
        body_q = state.body_q.numpy()
        body_q[self.piper_tennis_body] = xform_np
        state.body_q.assign(body_q)

        body_qd = state.body_qd.numpy()
        body_qd[self.piper_tennis_body] = 0.0
        state.body_qd.assign(body_qd)

        joint_q = state.joint_q.numpy()
        joint_q[self.piper_tennis_joint_q_start : self.piper_tennis_joint_q_start + 7] = xform_np
        state.joint_q.assign(joint_q)

        joint_qd = state.joint_qd.numpy()
        joint_qd[self.piper_tennis_joint_qd_start : self.piper_tennis_joint_qd_start + 6] = 0.0
        state.joint_qd.assign(joint_qd)

    def sync_backend_tennis_body_xform(self, body_xform):
        self.current_tennis_body_xform_np = np.asarray(
            [
                body_xform[0],
                body_xform[1],
                body_xform[2],
        ...
        for state in (self.piper_state_0, self.piper_state_1):
            self._write_backend_tennis_state(state, self.current_tennis_body_xform_np)
    ...
    def simulate_piper(self):
        ...
        self._write_backend_tennis_state(self.piper_state_1, self.current_tennis_body_xform_np)
        self.backend_gripper_has_tennis_contact = self._has_backend_gripper_tennis_contact()

        piper_body_q_out = self.piper_state_1.body_q.numpy()
        ...
        self.sync_frontend_piper_display_state(self.piper_state_0, self.piper_state_1)
        self.piper_state_0, self.piper_state_1 = self.piper_state_1, self.piper_state_0
    ...
    def simulate(self):
        ...
        if self.world.fetch_sim(0):
            ...
            for body_idx, shape_idx, shape_label in self.dynamic_sim_rigid_body_shapes:
                ...
                self.sync_backend_tennis_body_xform(body_xform)
